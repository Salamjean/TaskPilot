...
@section('content')
    ...
    <div class="at-card">
        <div
            ...
            <h2 style="font-size:1.2rem;font-weight:800;color:#111827;">Mon Journal de Présence</h2>
            <span style="font-size:.82rem;color:#9CA3AF;">{{ $attendances->total() }}
                pointage{{ $attendances->total() > 1 ? 's' : '' }}</span>
        </div>
        ...
        <form method="GET" action="{{ request()->url() }}">
            <div class="at-filter-row">
                ...
                <div style="display:flex;gap:8px;align-self:flex-end;">
                    <button type="submit" class="at-filter-btn at-filter-btn-primary">Filtrer par date</button>
                    @if(($dateFrom ?? '') || ($dateTo ?? ''))
                        <a href="{{ request()->url() }}" class="at-filter-btn at-filter-btn-reset">✕ Réinitialiser</a>
                    @endif
                </div>
            </div>
        </form>
                    ...
                    @forelse($attendances as $at)
                        <tr>
                            <td style="text-align:center;font-weight:600;color:#111827;">
                                {{ $at->date->translatedFormat('l d M Y') }}</td>
