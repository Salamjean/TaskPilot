...
@section('content')
    ...
    @php
        ...
        $todayCount = DailyLog::whereDate('date', now()->toDateString())->count();
        $totalCount = DailyLog::count();
        $usersCount = DailyLog::whereDate('date', now()->toDateString())->distinct('user_id')->count();
    @endphp
    ...
    <div class="dl-stats">
        ...
        <div class="dl-stat">
            ...
            <div class="dl-stat-value" style="color:#7E22CE;">{{ $usersCount }}</div>
        </div>
        <div class="dl-stat">
    ...
    <form method="GET" class="dl-toolbar">
        ...
        <a href="{{ route('prestataire.daily-logs.index') }}" class="dl-filter-reset">Réinitialiser</a>
    </form>
            ...
            @foreach($logs as $log)
                @php
                    ...
                    $linkedTasks = $tasksCount > 0
                        ? \App\Models\Task::with('project')->whereIn('id', $log->linked_task_ids)->get()
    ...
    <script>
        function showLog(log) {
            ...
            if (log.tasks && log.tasks.length > 0) {
                tasksHtml = `<div style="margin-top:16px;"><div style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#6B7280;letter-spacing:.5px;margin-bottom:8px;">Tâches liées</div><div style="display:flex;flex-direction:column;gap:6px;">`;
                log.tasks.forEach(t => {
                    let badge = t.done
                    ...
                    tasksHtml += `<div style="display:flex;align-items:center;justify-content:space-between;background:#F9FAFB;padding:8px 12px;border-radius:10px;border:1px solid #E5E7EB;"><div><div style="font-size:.85rem;font-weight:600;color:#1F2937;">${t.title}</div><div style="font-size:.73rem;color:#9CA3AF;">${t.project}</div></div>${badge}</div>`;
                });
                tasksHtml += `</div></div>`;
            }
            Swal.fire({
                title: `<span style="font-size:.85rem;color:#6B7280;font-weight:600;">${log.user}</span><br><span style="font-size:1rem;">📅 ${log.date}</span>`,
                html: `<div style="text-align:left;"><div style="background:#F9FAFB;border-radius:12px;border:1px solid #E5E7EB;padding:16px;white-space:pre-wrap;font-size:.9rem;color:#374151;line-height:1.7;">${log.content}</div>${tasksHtml}</div>`,
