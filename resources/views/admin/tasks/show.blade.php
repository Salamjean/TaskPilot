...
@section('content')
    ...
    <nav class="ts-bc">
        <a href="{{ route($prefix . '.tasks.index') }}">Tâches</a>
                ...
                <tbody>
                    @foreach($project->tasks as $task)
                                ...
                                <td>
                                    @if($task->assignee)
                                        <div style="display:flex;align-items:center;gap:7px; justify-content: center;">
                                            <div
                                                style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,var(--secondary),var(--accent));display:flex;align-items:center;justify-content:center;font-size:.68rem;font-weight:700;color:#fff;flex-shrink:0;overflow:hidden;">
                            ...
                            'is_verified' => (bool) $task->is_verified
                                        ...
                                        @if($task->status === 'termine' && in_array(auth()->user()->role, ['admin', 'responsable']))
                                            <form method="POST" action="{{ route($prefix . '.tasks.update', $task) }}">
                                                @csrf @method('PUT')
                                                <input type="hidden" name="is_verified" value="{{ $task->is_verified ? '0' : '1' }}">
                                                <button type="submit" class="ts-action-btn"
                                                    style="{{ $task->is_verified ? 'background:#ECFDF5;color:#059669;border-color:#A7F3D0;' : 'background:#ECFDF5;color:#059669;border-color:#A7F3D0;' }}">
                                                    @if($task->is_verified)
                                        ...
                                        @if($task->status === 'termine' && in_array(auth()->user()->role, ['admin', 'responsable']))
                                            <form method="POST" action="{{ route($prefix . '.tasks.reschedule', $task) }}"
                                        ...
                                        @if($task->status !== 'termine' && in_array(auth()->user()->role, ['admin', 'responsable']))
                                            <form method="POST" action="{{ route($prefix . '.tasks.destroy', $task) }}"
                                                onsubmit="return confirm('Supprimer cette tâche ?')">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="ts-action-btn danger">Supprimer</button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
    ...
    <div class="ts-modal-backdrop" id="addModal">
        <div class="ts-modal" onclick="event.stopPropagation()">
            ...
            <form method="POST" action="{{ route($prefix . '.tasks.store', $project) }}">
                @csrf
